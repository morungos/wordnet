...
find = function(searchKey, callback) {
  var indexFile;
  indexFile = this;
  return indexFile.open(function(err, fd, done) {
    var pos, size;
    if (err) {
...
lookupFromFile = function(word, callback) {
  return this.find(word, function(record) {
    ...
    if (record.status === 'hit') {
      ptrs = [];
      offsets = [];
      for (i = _i = 0, _ref = parseInt(record.tokens[3]) - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
        ptrs.push(record.tokens[i]);
      }
      for (i = _j = 0, _ref1 = parseInt(record.tokens[2]) - 1; 0 <= _ref1 ? _j <= _ref1 : _j >= _ref1; i = 0 <= _ref1 ? ++_j : --_j) {
