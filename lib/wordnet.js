...
lookup = function(word, pos, callback) {
  var selectedFiles, wordnet, _ref;
  wordnet = this;
  word = word.toLowerCase().replace(/\s+/g, '_');
  if (callback == null) {
    _ref = [false, pos], pos = _ref[0], callback = _ref[1];
  }
  selectedFiles = !pos ? wordnet.allFiles : wordnet.allFiles.filter(function(file) {
    return file.pos === pos;
  });
  return wordnet.lookupFromFiles(selectedFiles, [], word, callback);
};
